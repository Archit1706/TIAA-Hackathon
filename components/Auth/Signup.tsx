...
const Signup = (props: Props) => {
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            const response = await fetch("https://auction-backend.sidd065.repl.co/api/users/register",
                {
                    method: "POST",
                    headers: {
                ...
                <div className="w-full flex flex-col items-center justify-center px-6 py-8 mx-auto pl-28 md:h-screen lg:py-0">
                    <div className="w-[500px] bg-white rounded-lg drop-shadow-lg">
                        <div className="space-y-2 py-10 px-10">
                            <h1 className="text-2xl pb-3 text-center font-bold leading-tight tracking-tight text-teal-400 md:text-2xl">
                                Sign Up
                            </h1>
                            <form onSubmit={handleSubmit} className="space-y-4 md:space-y-4" >
                                <div>
                                    <label htmlFor="name" className="block mb-2 text-base font-medium text-gray-900">User name</label>
                                ...
                                <div>
                                    <label htmlFor="mobile" className="block mb-2 text-base font-medium text-gray-900">Mobile number</label>
                                    <input value={mobile} onChange={(e) => setMobile(e.target.value)} type="text" name="mobile" id="name" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Enter your mobile number" required />
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <input value={password} onChange={(e) => setPassword(e.target.value)} type="password" name="password" id="password" placeholder="Create password of atleast 6 characaters" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " required />
                                </div>
                                <div>
                                    <label htmlFor="addres" className="block mb-1 text-base font-medium text-gray-900">Address</label>
                                    <textarea value={address} onChange={(e) => setAddress(e.target.value)} name="address" id="password" placeholder="Write your address..." rows={2}
                                ...
                                <button type="submit" className="w-full text-white bg-teal-400 hover:bg-teal-500 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-lg px-5 py-2.5 text-center">
                                    Sign Up
                                </button>
                                <hr />
