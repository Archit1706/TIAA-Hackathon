...
const FAQ = ({ QnA, product }: Props) => {
    ...
    return (
        <div className="max-h-[70vh] overflow-auto w-full mx-auto px-5 pb-5 bg-white rounded-lg shadow-lg font-body">
            <ToastContainer />
            {
                showQueryModal && <QueryModal QnA={QnA} product={product} setShowQueryModal={setShowQueryModal} />
            }
            <div className="flex items-center justify-between sticky top-0 bg-white w-full p-4 shadow-sm">
                ...
                <button
                    ...
                    className="text-base hover:scale-110 focus:outline-none flex justify-center px-4 py-2 rounded font-bold cursor-pointer hover:bg-mobile-light bg-mobile-light text-mobile border duration-200 ease-in-out border-mobile transition">Query</button>
            ...
            <div className="mt-8 space-y-8">
                {
                    QnAs && Array.isArray(QnAs) && QnAs.map((item, index) => (
                        <div>
...
export const ResponseCrad = ({ product, QnA, setQnAs, index } : ResponseCradProps) => {
    const [answer, setAnswer] = useState("")
    const submitAnswer = () => {
        if (answer.length > 5) {
            // submit answer to backend
