...
router.post("/predict-soldDate", async (req, res) => {
    try {
			const product= await Product.find({category:req.body.category}).sort({ price: -1 });
...
router.post("/predict", async (req, res) => {
    try {
			...
			switch(model) {
  case 'Mobiles':
    axios.post('https://auction-backend-ml.sidd065.repl.co/api/predict/mobile',{
...
router.post("/search", async (req, res) => {
    try {
      const products = await Product.find({category:req.body.category, name:req.body.name});
...
router.post("/add-question", async (req, res) => {
    try {
      const product = await Product.findById(req.body.id);
			const comment = {name: req.body.name, comment: req.body.comment, response: ""};
...
router.post("/add-review", async (req, res) => {
    try {
      const product = await Product.findById(req.body.id);
			const comment = {name: req.body.name, comment: req.body.comment, rating: req.body.rating};
    ...
    try {
      const product = await Product.findById(req.body.id);
			...
			for(let i = 0; i<productQuestions.length; i++){
					if (productQuestions[i].comment==req.body.comment && productQuestions[i].name==req.body.name){
