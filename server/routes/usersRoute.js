...
router.post("/admin", async (req, res) => {
    try {
			...
			//if(user.isAdmin){
				...
				res.send({maxValue:maxValue, maxBids:maxBids, maxRatings:maxRatings, success: true});
			//}
...
router.post("/dashboard", async (req, res) => {
  try {
		...
		const likedProducts = await Product.find({'_id': { $in: user.likedProducts}})
		const likedUsers = await Product.find({'_id': { $in: user.likedUsers}})
		res.send({
      itemsBought:itemsBought, itemsSold:itemsSold, likedProducts:likedProducts, likedUsers:likedUsers,
      success: true,
    });
  } catch (error) {
    res.send({
